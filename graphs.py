...
print(df.head())
top_10 = (df.groupby('Name')['Course', 'Time'].agg({
    'Course': 'count',
    'Time': 'min'
}).sort_values(by='Time', ascending=True))[:10].reset_index()
...
top_10.plot(kind='barh', y='Runs', x='Name', ax=ax)
# ax.set_xlim([40000, 140000])
ax.set(title='Number of parkruns', xlabel='Name', ylabel='Runner')
...
# Count plot
top_10.plot(kind='barh', y='Runs', x='Name', ax=ax0)
# ax0.set_xlim([-10000, 140000])
ax0.set(title='Revenue', xlabel='Total Revenue', ylabel='Customers')
# ax0.axvline(x=avg, color='b', label='Average', linestyle='--', linewidth=2)

# Unit sales plot
top_10.plot(kind='barh', y='Runs', x='Name', ax=ax1)
